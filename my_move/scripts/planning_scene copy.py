...
def teste1(move_group, pose_goal):
    move_group.set_goal_tolerance(0.15)
    ...
    print("Current robot_state joint values", robot_state.joint_state.position)
    ...
    while not start_position_confirmed:
        start_position_confirmed = move_group.go(wait=True)
    robot_state = robot.get_current_state()
    print("state after going to start position", robot_state)
    # move_group.set_start_state_to_current_state()
    joints = [-150, 84, 45, -118, 74, 118]
    ...
    moveit_robot_state = robot.get_current_state().joint_state

    joint_state = JointState()
    joint_state.header.stamp = rospy.Time.now()
    joint_state.name = moveit_robot_state.name
    joint_state.position = joint_start + list(moveit_robot_state.position[6:])
    # print("robot_state after changes--", joint_state)
    ...
    return len(plan[1].joint_trajectory.points), plan[2]
...
if __name__ == '__main__':
    ...
    for i in range(1,5):
        results = teste1(move_group, pose_goal)
        paths_lenght.append(results[0])
        planning_time_list.append(round(results[1], 4))

    print("25 list waypoints", paths_lenght)
    print("25 list planning times", planning_time_list)
    print("25 list times from start", [round(i, 4) for i in times_from_start])
    ...
    # print("50 list waypoints", paths_lenght)
    # print("50 list planning times", planning_time_list)
    # print("50 list times from start", [round(i, 4) for i in times_from_start])
